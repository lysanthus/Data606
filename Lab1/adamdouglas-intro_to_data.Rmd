...
```{r load-data, eval=TRUE, cache=TRUE}
source("more/cdc.R")
```
...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    There are 20,000 cases in the dataset and 9 variables. The variables are:
    * genhlth - a nominal categorical variable
    * exerany - a nominal categorical variable
    * hlthplan - a nominal categorical variable
    * smoke100 - a nominal categorical variable
    * height - a discrete numeric variable
    * weight - a discrete numeric variable
    * wtdesire - a discrete numeric variable
    * age - a discrete numeric variable
...
table(cdc$gender)
```
    There are 9,569 males in the dataset.
    
```{r}
prop.table(table(cdc$genhlth))
```
    23.285% of the sample report excellent health.
...
3.  What does the mosaic plot reveal about smoking habits and gender?
    ...
    The mosaic plot tells us that (slightly) less females in the sample are smokers.
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    This box plot shows that participants who claimed better health had generally lower BMIs.. Below I will look at how BMI relates to age groups (by 10 years). I believe that our BMI increases as we age and eventually levels out. However, the plot shows a decrease in median BMI as we get past age 70 - perhaps the shrinking size is offset by a loss in weight.
    
```{r}
boxplot(bmi ~cut(cdc$age, breaks=c(0,20,30,40,50,60,70,80,90,100,110)))
```
...
```{r}
library(ggplot2)
ggplot(cdc, aes(x=weight, y=wtdesire)) + geom_point(alpha=0.2) + ylab("Desired Weight") + xlab("Current Weight")
```
    
    There appears to be a linear relationship between the participant's current weight and their desired weight.
...
```{r}
wdiff <- cdc %>% mutate(wdiff = wtdesire - weight) %>% select(wdiff)
```
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    `wdiff` is a discrete numeric variable. If it is positive then the person wishes to gain weight, while a negative value represents a desire to lose weight. If a value is zero, then the person desires to neither lose nor gain any weight.
...
```{r}
ggplot(wdiff) + geom_density(mapping = aes(wdiff))
```
    
    The distribution of `wdiff` is centered around 0. A much larger portion of the distribution lies in the negative rather than positive direction, indicating that more people desire to lose weight than gain weight. There are, however, a few extreme values in the data where people are looking to lose (or gain!) more than 100lbs:
    
```{r}
wdiff %>% filter(wdiff < -100 | wdiff > 100) %>% summarize(n())
```
...
```{r}
# Add our difference to the main CDC dataset
cdc <- mutate(cdc, diff = wtdesire - weight)

# Numerical summary
cdc %>% group_by(gender) %>% summarize(median = median(diff), IQR = IQR(diff))

# Box plot
ggplot(cdc, aes(gender,diff)) + geom_boxplot()
```
    
    It appears that men, typically, are less concerned with their weight than women.
...
```{r}
cdc %>% mutate(z1 = abs(scale(weight)) <= 1) %>% group_by(z1) %>% summarize(prop=n()/20000)
```
  Just over 70% weights are within 1 standard deviation from the mean.    
