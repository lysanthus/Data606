...
samp.mean <- mean(samp)
samp.med <- median(samp)
```

**The distribution of this sample is definitely right-skewed. The mean of the distribution is `r format(round(samp.mean,1), scientific = FALSE, big.mark = ",")` and the median is `r format(round(samp.med,1), scientific = FALSE, big.mark = ",")`. Because of the skew I'd tend to use the median as the "typical" size.**

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

**I would not expect another student's distribution to look like mine. They may be similar in some respects, but there will be differences. For example, they could end up with the 60 largest values from the population in their sample whereas I may end up with the 60 smallest; because it is a random sample there will always be some difference.**
...
**This typically holds true when the population is not strongly skewed and the sample is greater than 30.**

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

**A 95% confidence interval means that if we took 100 samples and looked at the range given by that $\bar{x} \pm 1.96\times(se)$, about 95 of them would contain the mean.**
...
**I'd expect about 95% of them to have the mean in their confidence interval, because that is how we define the confidence interval at 95% - that approximate proportion of confidence intervals should contain the mean.**
...
**No, it does not match the exact confidence level (92% vs. 95%). This is because there is, again, variability in which samples we get. THis could also be because the population is not exactly normal and we are estimating population standard deviation with the sample standard deviation.**

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?

```{r}
crit <- round(abs(qnorm(0.005,0,1)),2)
```

**Choosing 99% confidence interval, the critical value is `r crit`.** 
...
**With a 99% confidence interval, I'd expect 1 or 0 misses. I got 2 misses which equates to 96%, which is a bit high.**
