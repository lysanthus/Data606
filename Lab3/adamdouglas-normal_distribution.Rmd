...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
  ...
  **This data does appear to follow a nearly-normal dsitribution.**
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    **Yes. In fact, if they weren't labelled, I wouldn't be able to decide which represented actual data, and which is simulated data.**
...
```
    ...
    **It appears that female weights are not quite near-normal.**
    **In fact they seem a little bit right-skewed**
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    **What is the probability that a randomly-chosen female has a height between 160 and 170?**
    
```{r}
# Theoretical
pnorm(q=170, mean = mean(fdims$hgt), sd = sd(fdims$hgt)) -
  pnorm(q=160, mean = mean(fdims$hgt), sd = sd(fdims$hgt))

# Empirical
sum(fdims$hgt <= 170 & fdims$hgt >= 160) / length(fdims$hgt)
```
    
**What is the probability that a randomly-chosen female has a**
**weight greater than 80 *or* a weight less than 50**
    
```{r}
# Theoretical
1 - (pnorm(q=80, mean = mean(fdims$wgt), sd = sd(fdims$wgt)) -
  pnorm(q=50, mean = mean(fdims$wgt), sd = sd(fdims$wgt)))

# Empirical
sum(fdims$wgt <= 50 | fdims$wgt >= 80) / length(fdims$wgt)
```
...
** This is likely because the values are more discrete and less continuous**
...
**The distribution of `kne.di` appears to be right-skewed.**

```{r}
hist(fdims$kne.di, main="Female Knee Diameter",xlab="Diameter")
```
