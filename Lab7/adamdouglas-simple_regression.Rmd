...
**The relationship between `at_bats` and `runs` is somewhat linear, though I would not be comfortable using only that variable to predict `runs`.**
...
**As `at_bats` increases, `runs` also increases, showing a positive correlation. The relationship is somewhat linear (correlation coefficient of 0.6106).**
...
**The equation of the regression line is $\hat{y} = 415.2389 + 1.8345 * atbats$. The slope tells us that for every homerun the team earns, we expect to see approximately 1.8345 runs.**
...
**The manager would predict `r predict(m1, data.frame(at_bats=5578))` runs. The manager may be over or under estimating, there is no way to know. Because it is not an actual value there is no residual.**
...
**There isn't any apparent pattern in the residual plot, so the linearity seems to hold.**
...
**The residuals are not as normal as I'd like to see, but for large values of "nearly" they may be "nearly normal".**
...
**Yes. The variability in residuals appears to be more or less constant.**
...
```{r}
model1 <- lm(runs ~ hits, data=mlb11)
plot(runs ~ hits, data=mlb11)
```

**Here it looks like hits has a positive linear relationship with runs.**
...
**The R-squared value for this new model (`hits` and `runs`) is similar to the original one (`at_bats` and `runs`).**
...
```{r}
model2 <- lm(runs ~ homeruns, data = mlb11)
model3 <- lm(runs ~ bat_avg, data = mlb11)
model4 <- lm(runs ~ strikeouts, data = mlb11)
model5 <- lm(runs ~ stolen_bases, data = mlb11)
model6 <- lm(runs ~ wins, data = mlb11)

r2 <- data.frame(model = 1, rsq = summary(model1)$r.squared)
r2 <- rbind(r2, data.frame(model = 2, rsq = summary(model2)$r.squared))
r2 <- rbind(r2, data.frame(model = 3, rsq = summary(model3)$r.squared))
r2 <- rbind(r2, data.frame(model = 4, rsq = summary(model4)$r.squared))
r2 <- rbind(r2, data.frame(model = 5, rsq = summary(model5)$r.squared))
r2 <- rbind(r2, data.frame(model = 6, rsq = summary(model6)$r.squared))

r2[which(r2$rsq == max(r2$rsq)),]
```
**The best model is the third one (using `bat_avg` as the predictor variable), however it is not better than others by any kind of significant amount.**
...
```{r}
model7 <- lm(runs ~ new_onbase, data = mlb11)
model8 <- lm(runs ~ new_slug, data = mlb11)
model9 <- lm(runs ~ new_obs, data = mlb11)

r2 <- rbind(r2, data.frame(model = 7, rsq = summary(model7)$r.squared))
r2 <- rbind(r2, data.frame(model = 8, rsq = summary(model8)$r.squared))
r2 <- rbind(r2, data.frame(model = 9, rsq = summary(model9)$r.squared))

r2[7:nrow(r2),]
```

**The model with the highest R-squared value is the model that uses the new on-base measure `new_obs`. I don't know what "slugging" is, so I can't comment if this combined with on-base percentage makes sense.**
...
```{r}
# Check Linearity
ggplot(mlb11, aes(x=new_obs, y=runs)) + geom_point() + xlab("New On-Base") +
  ylab("Runs") + geom_line(aes(y=model9$fitted.values), col="red", linetype=3) +
  ggtitle("Linearity")

# Residual Plots
ggplot(mlb11, aes(x=mlb11$new_obs, y=model9$residuals)) + geom_point() +
  geom_line(aes(y=0), linetype=3) + ylab("Residual") + xlab("New On-Base") +
  ggtitle("Residual Plot")

ggplot(data.frame(resid = model9$residuals), aes(x=resid)) + geom_histogram(bins=10)

```

**Linearity, residual near-normality, and variability all seems to be appropriate for this model.**
