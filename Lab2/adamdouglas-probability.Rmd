...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    <span style="color:blue">*A streak of 1 means that there was only 1 hit before a miss took place. A streak of 0 is simply a miss (i.e. no hits).*</span>
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    <span style="color:blue">*Kobe's shooting looks like I'd expect it to. He has more streaks of length 0 and 1 than any other, and only a few of 3 and 4. His typical (mean) streak is `r round(mean(kobe_streak),2)` while his longest is `r max(kobe_streak)`. Nothing here makes me believe he had any kind of "hot hands".*</span>
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    <span style="color:blue">*In the unfair ccoin simulation, there were `r length(sim_unfair_coin[sim_unfair_coin == "heads"])` heads outcomes.*</span>
...
```{r}
# Get the streaks from the simulated shooter
...
```{r}
# Plot the simulated shooter streaks
barplot(table(sim_streak))
```

<span style="color:blue">*The typical (mean) streak for the simulated shooter is `r round(mean(sim_streak))` and the longest streak is `r max(sim_streak)`.*</span>
...
<span style="color:blue">*I would expect the  distribution to be slightly different each time it is run. Occasionally, it may be significantly different, but for the most part it should be comparable each time. This is because there is some randomness to the hit and miss percentages. A 45% hit percentage doesn't mean that there will always be 45 hits in 100 shots, it only means that for a **large** number of simulations the mean number of hits will approach 45.*</span>
...
# Combine the streak data
combined <- full_join(data.frame(table(kobe_streak)),data.frame(table(sim_streak)),by=c("kobe_streak" = "sim_streak"))
colnames(combined) <- c("streak","kobe","sim")

# Plot the results side-by-side
gather(combined, kobe, sim, key="shooter", value="num") %>% ggplot(aes(streak,num)) + geom_col(aes(fill=shooter), position="dodge") + xlab("Hit Streak") + ylab("# Streaks") + ggtitle("Kobe Bryant Shot Streaks vs. Simulation","2009 NBA Finals vs. Orlando (n=133)") + labs(fill="Shooter")
```

<span style="color:blue">*I see no convincing evidence that Kobe Bryant had "hot hands" during these games. His streak performance compares somewhat well when placed side-by-side with the simulation. In some cases the simulated shooter has a few longer streaks that Kobe did not.*</span>
