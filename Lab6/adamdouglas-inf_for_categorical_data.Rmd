...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    **The findings are sample statistics, as all people in the world were surely not polled for this study.**
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    **We have to assume that the study was the result of a random sample and that those samples were representative of the larger global population. It seems like a reasonable assumption given the considerable sample sizes used (page 15).**
...
**Each row of Table 6 corresponds with the results of sample within the country, while each row of the `atheism` data corresponds to a single response in the survey.** 
...
**Yes our calculated proportion agrees with the survey.**
...
**The conditions we need to construct a confidence interval are: independence and the success-failure condition. For independence, we assume that the samples are random, as each comes from a different person. For the success-failure condition, we check to see that there are at least 10 successes and 10 failures in the population by using the sample proportion as an estimator. Looking at the data we have `r nrow(us12[us12$response=="atheist",])` "successes" and `r nrow(us12[us12$response!="atheist",])` "failures"**
...
**The margin of error in the R output is `r 0.0634 - 0.0499`.**
...
```{r}
# Get data for our countries
india12 <- subset(atheism, nationality == "India" & year == "2012")
austria12 <- subset(atheism, nationality == "Austria" & year == "2012")

# Check proportions for success-failure
pIndia <- nrow(india12[india12$response=="atheist",])
qIndia <- nrow(india12[india12$response!="atheist",])
pIndia
qIndia

pAustria <- nrow(austria12[austria12$response=="atheist",])
qAustria <- nrow(austria12[austria12$response!="atheist",])
pAustria
qAustria
...
inference(austria12$response, est = "proportion", type = "ci", 
          method = "theoretical", success = "atheist")
```

**The conditions for inference are met (independence and number of successes)**
...
**As the proportion moves from the extremes to 0.05 the margin of error increases.**
...
**The sampling distribution is nearly normal and mostly symmetric, centered at `r mean(p_hats)`.**
...
# Get plot from above
plot1 <- hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
...
plot2 <- hist(p_hats, main = "p = 0.1, n = 400", xlim = c(0, 0.18))
...
plot3 <- hist(p_hats, main = "p = 0.2, n = 1040", xlim = c(0.12, 0.30))
...
plot4 <- hist(p_hats, main = "p = 0.2, n = 400", xlim = c(0.12, 0.30))

```

**A larger proportion spreads out the distribution more, which aligns with the formula for margin of error. A smaller $n$ seems to make the margin of error greater and less-normal.**
...
**I would say it is fair to proceed with the inference**
...
**First we check conditions for inference. We already trust that the study has independent responses so next we check to see if the success-failure condition is met for the data sets

```{r}
pSpain05 <- nrow(spain05[spain05$response=="atheist",])
qSpain05 <- nrow(spain05[spain05$response!="atheist",])

pSpain12 <- nrow(spain12[spain12$response=="atheist",])
qSpain12 <- nrow(spain12[spain12$response!="atheist",])
```

**We have at least 10 successes and failures for both data sets, so we should be ok to assume normality of the sampling distributions.**

**Our hypothesis is that the sample mean of the 2012 distribution is the same as it was in 2005 and the difference is merely due to random chance. Said mathematically:**

$$H_{0}:\mu_{2012}=\mu_{2005} \text{  ; }H_{A}:\mu_{2012}\ne\mu_{2005}$$
Now we check the confidence intervals to see if there is overlap.
...
**Looking at the confidence intervals, they overlap considerably. So, there is not enough compelling evidence to reject the null hypothesis and we continue to accept that the differences we are seeing in $\hat{p}$ are due to random chance and not indicative of a change in the population proportion ($p$).**
...
pUS05 <- nrow(us05[us05$response == "atheist",])
...
**Yes, there is convincing evidence of a change in population proportion. Comparing the confidence interval for 2005 with the one from 2012 we calculated above in #5, there is no overlap at the 95% confidence interval.**
...
**Since we do not know what to expect for $p$, we use the worst-case of 0.50 because, as shown above, that gives us the worst margin of error. We can then calculate the minimum sample size to achieve this maximum level of error:**

$$1.96^2\times\frac{0.5^2}{n}\lt0.01^2$$
$$3.8416\times\frac{0.025}{0.0001}\lt n = 960.4$$
**We would need a sample size of at leaast 961 to ensure a margin of error no greater than 0.01.**
